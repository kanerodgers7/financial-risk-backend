  ...
  try {
    ...
    switch (req.body.stepper) {
      ...
      case 'confirmation':
        ...
        if (application && application.note && application.clientId._id) {
          await addNote({
            userType: 'client-user',
            userId: req.user.clientId,
            userName: application.clientId.name,
        ...
        await addAuditLog({
          ...
          logDescription: `A new application ${application.applicationId} is successfully generated by ${application.clientId.name}`,
        });
        checkForAutomation({
