...
const storage = multer.diskStorage({
    destination: function (req, file, cb) {
        cb(null, uploadPath + '/' + getImagePath({documentFor: req.query.documentFor}));
    },
    filename: function (req, file, cb) {
        cb(null, req.query.documentFor + '-' + file.fieldname + '-' + Date.now() + file.originalname.substr(file.originalname.lastIndexOf('.'), file.originalname.length));
    },
});
...
router.get('/column-name',async function (req,res) {
    ...
    try {
        ...
        const auditLogsColumn = req.user.manageColumns.find(i => i.moduleName === 'document');
        let customFields = [];
        let defaultFields = [];
        ...
        Logger.log.error('Error occurred in get document column names', e.message || e);
...
router.get('/:entityId', async function (req, res) {
    if (!req.query.documentFor || !req.params.entityId || !mongoose.Types.ObjectId.isValid(req.params.entityId)) {
    ...
    try {
        const module = StaticFile.modules.find(i => i.name === 'document');
        const documentColumn = req.user.manageColumns.find(i => i.moduleName === 'document');
        ...
        if(req.query.documentFor === 'application'){
            ...
            query = {
                $and: [{
                    ...
                    $or: [{uploadByType: 'client-user', uploadById: application.clientId},
            ...
            query = {
                $and: [{
                    ...
                    $or: [{uploadByType: 'client-user', uploadById: debtor.clientId},
                        {uploadByType: 'user', isPublic: true},
                        {uploadByType: 'user', uploadById: req.user._id}]
                }]
            }
            ...
            query = {
                $and: [{
                    ...
                    $or: [{uploadByType: 'user', isPublic: true},
                        {uploadByType: 'user', uploadById: req.user._id}]
                }]
            }
        }
    ...
    try {
        ...
        res.status(200).send({
            ...
            data: {
                ...
                filename: getImageUrl({imageName: req.file.filename, documentFor: req.query.documentFor}),
                originalName: req.file.originalname
            }
        });
        if (req.query.oldFileName) {
            Logger.log.info('Old image name:', req.query.oldFileName);
            await deleteImage({fileName:req.query.oldFileName,filePath:uploadPath + '/' + getImagePath({documentFor: req.query.documentFor})});
        }
    } catch (e) {
...
router.post('/', async function (req, res) {
    if (!req.body.documentType || !req.body.description || !req.body.originalName || !req.body.fileName || !req.body.entityType || !req.body.entityId || !req.body
        .hasOwnProperty('isPublic')) {
        return res.status(400).send({
            status: 'ERROR',
            messageCode: 'REQUIRE_FIELD_MISSING',
            message: 'Require fields are missing.'
    ...
    try {
        ...
        res.status(200).send({status: 'SUCCESS', message: 'Document added successfully.'});
    } catch (e) {
...
router.put('/column-name',async function (req,res) {
    ...
    try {
        ...
        await User.updateOne({_id:req.user._id,'manageColumns.moduleName':'document'},{$set:{'manageColumns.$.columns':updateColumns}});
...
router.post('/:documentId', async function (req, res) {
    ...
    try {
        ...
        res.status(200).send({status: 'SUCCESS', message: 'Document updated successfully.'});
    } catch (e) {
        Logger.log.error('Error occurred in add document ', e.message || e);
        res.status(500).send({status: 'ERROR', message: e.message || 'Something went wrong, please try again later.'});
    }
...
router.delete('/:documentId', async function (req, res) {
    if (!req.params.documentId || !mongoose.Types.ObjectId.isValid(req.params.documentId)) {
        return res.status(400).send({
            status: 'ERROR',
            messageCode: 'REQUIRE_FIELD_MISSING',
            message: 'Require fields are missing.'
    ...
    try {
        ...
        await deleteImage({fileName:document.fileName,filePath:uploadPath + '/' + getImagePath({documentFor: document.documentTypeId.documentFor})});
        await Document.updateOne({_id:req.params.documentId},{isDeleted:true});
...
function getImagePath({documentFor}) {
    return config.uploadLocations[documentFor]['base'] + config.uploadLocations[documentFor]['document'];
}

function getImageUrl({imageName, documentFor}) {
    if (imageName)
        if (imageName.indexOf(config.server.backendServerUrl + getImagePath({documentFor})) !== -1) return imageName;
        else return config.server.backendServerUrl + getImagePath({documentFor}) + '/' + imageName;
    return '';
}
