    ...
    jwt.verify(req.body.token, config.jwt.secret, async (err, decoded) => {
        if (err) {
            ...
            if (decoded.expiredTime < Date.now()) {
                res.status(401).send({message: 'The link to reset password has expired, please repeat the process by clicking on Forget Password from login page.'});
            ...
            if (decoded._id.toString() !== req.params.id.toString()) {
                ...
                try {
                    ...
                    if (!clientUser) {
                        ...
                        return res.status(400).send({message: 'Password has already once set, to recover password, click on Forgot Password from Login Page.'});
