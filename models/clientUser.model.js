    ...
    try {
        ...
        if (clientUserData.jwtToken.indexOf(token) !== -1) {
            if (decoded.expiredTime > d.getTime()) {
                ...
                clientUserData.jwtToken.splice(clientUserData.jwtToken.indexOf(token), 1);
...
clientUserSchema.methods.getAuthToken = function () {
    ...
    const access = 'auth';
    return jwt.sign({ _id: a._id.toHexString(), expiredTime: parseInt(config.jwt.expireTime) * 3600000 + d.getTime(), access }, jwtSecret).toString();
};
