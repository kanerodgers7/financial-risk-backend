...
let individualList = fs.readFileSync(path.join(...inputFilePath, 'individual-list.json'));
individualList = JSON.parse(individualList.toString());

let addressList = fs.readFileSync(path.join(...inputFilePath, 'address-list.json'));
addressList = JSON.parse(addressList.toString());

let questionAnswerList = fs.readFileSync(path.join(...inputFilePath, 'question-answer.json'));
...
let applicationDetails = fs.readFileSync(path.join(...inputFilePath, 'application-details.json'));
...
const storeMerchantCode = async () => {
  try {
    let clientList = fs.readFileSync(path.join(...inputFilePath, 'client.json'));
  ...
  try {
    for (let key in applicationList) {
      ...
      if (
        ...
        if (allowedApplicationStatus.indexOf(applicationList[key][activeApplicationIndex]['cd_status']) !== -1 && allowedLimitTypes.indexOf(activeApplicationDetails['tx_tcr_product']) !== -1) {
          ...
          if (!client) {
            ...
            if (debtor) {
              ...
              application.creditLimit = approvedApplicationDetails?.['am_requsted']
                ? parseInt(approvedApplicationDetails['am_requsted'])
                : 0;
              application.acceptedAmount = approvedApplicationDetails?.['am_granted']
                ? parseInt(approvedApplicationDetails['am_granted'])
                : 0;
              application.clientReference =
                approvedApplicationDetails?.['no_acc_reference'] || '';
              application.comments = activeApplicationDetails?.['tx_comments'] || '';
              application.requestDate = approvedApplicationDetails?.['dt_submit']
                ? moment(
